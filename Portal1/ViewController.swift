...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func setupScene(){
        let nodo = SCNNode()
        nodo.position = SCNVector3.init(0, 0, 0)
        ...
        leftDoorSide.position = SCNVector3.init((-length / 2)+(doorLengh / 2), 0, length / 2)
        leftDoorSide.eulerAngles = SCNVector3.init(0, -90.0.degressToRadians, 0)
        
        let rightDoorSide = createBox(isDoor: true)
        rightDoorSide.position = SCNVector3.init((length / 2) - (doorLengh / 2), 0, length / 2)
        ...
        let contraint = SCNLookAtConstraint(target: bottomWall)
        contraint.isGimbalLockEnabled = true
        
        let lightNode = SCNNode()
        lightNode.light = light
        lightNode.position = SCNVector3.init(0, 0.4, 0)
        lightNode.constraints = [contraint]
        nodo.addChildNode(lightNode)
        
        //Añadiendo nodos al nodo principal
         nodo.addChildNode(leftWall)
         nodo.addChildNode(rightWall)
         nodo.addChildNode(topWall)
         nodo.addChildNode(bottomWall)
         nodo.addChildNode(backWall)
         nodo.addChildNode(leftDoorSide)
         nodo.addChildNode(rightDoorSide)
